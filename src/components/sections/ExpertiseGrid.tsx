    ...
    <section className="py-32 bg-surface-base relative z-10" id="especialidades">
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="text-center mb-20">
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-display font-light text-white tracking-tight mb-6">
            Ecosistema de <span className="font-serif italic text-emerald-300">Diagnóstico e Cura</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-[minmax(300px,_auto)]">
          ...
          <motion.div className="md:col-span-8 flex flex-col">
            <BentoCard className="flex-1 flex flex-col justify-between group/card hover:border-primary/30 transition-colors">
              <div className="flex justify-between items-start mb-8">
                <div className="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center text-primary group-hover/card:bg-primary group-hover/card:text-slate-900 transition-colors duration-500">
                  <Dna size={32} strokeWidth={1.5} />
                </div>
                <span className="clinical-glass px-4 py-1 rounded-full text-xs text-primary font-display border-primary/20">Complex Care</span>
              </div>
              <div>
                <h3 className="text-3xl font-display font-semibold text-white mb-4">Especialidades Clínicas</h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {['Oncologia', 'Nefrologia', 'Cardiologia', 'Endocrinologia', 'Oftalmologia', 'Odontologia', 'Gastrologia'].map(spec => (
                    <span key={spec} className="px-3 py-1.5 rounded-full border border-white/5 bg-white/5 text-sm font-sans text-slate-300">
                      {spec}
                    </span>
                  ))}
                </div>
          ...
          <motion.div className="md:col-span-4 flex flex-col">
            <BentoCard className="flex-1 flex flex-col justify-between bg-gradient-to-br from-slate-800/40 to-emerald-900/10">
              <Search className="w-12 h-12 text-emerald-200 mb-8 opacity-80" strokeWidth={1.5} />
              <div>
                <h3 className="text-2xl font-display font-semibold text-white mb-3">Imagens & Lab</h3>
          ...
          <motion.div className="md:col-span-5 flex flex-col">
            <BentoCard className="flex-1 flex flex-col justify-between">
              <Flower2 className="w-12 h-12 text-teal-300 mb-8 opacity-80" strokeWidth={1.5} />
              <div>
                <h3 className="text-2xl font-display font-semibold text-white mb-3">Terapias Integrativas</h3>
                <p className="text-text-secondary font-sans leading-relaxed">
          ...
          <motion.div className="md:col-span-7 flex flex-col">
            <BentoCard className="flex-1 flex flex-col justify-between md:flex-row items-center gap-8">
              <div className="flex-1">
                ...
                <h3 className="text-3xl font-display font-semibold text-white mb-4">Clínica Geral & Cirurgia</h3>
              ...
              <div className="w-full md:w-48 h-48 rounded-full border-2 border-dashed border-white/10 flex items-center justify-center relative">
                <div className="absolute inset-0 rounded-full bg-primary/5 blur-xl animate-pulse" />
                <Activity className="w-16 h-16 text-primary/50" strokeWidth={1} />
              </div>
            </BentoCard>
          </motion.div>
